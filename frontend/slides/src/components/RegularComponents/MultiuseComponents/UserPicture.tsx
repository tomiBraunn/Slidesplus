...
export default function UserPicture({ avatar, size = 38 }: Props) {
  ...
  const fullName = `${localUser?.first_name || ""} ${localUser?.last_name || ""}`.trim() || "User";
  ...
  return (
    <div ref={containerRef} className="relative inline-block">
      ...
      <div
        className={`absolute right-0 mt-2 w- bg-[#0f0f0f] border border-[#2B2B2B] rounded-xl overflow-hidden transition-all duration-300 ease-out origin-top-right z-50 ${isOpen ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 -translate-y-2 pointer-events-none"
          }`}
      >
        <div className="p-6 pb-0 border-b border-[#2B2B2B] flex flex-col items-center gap-2 relative">
          ...
          <div className="relative z-10 flex flex-col items-center gap-2 pb-4">
            <div className="flex items-center gap-2">
              <div className="rounded-full overflow-hidden bg-gray-200 flex-shrink-0" style={{ width: 56, height: 56 }}>
                {src ? (
              ...
              <div className="flex flex-col min-w-0">
                ...
                <p className="text-base font-semibold text-gray-400 truncate text-xs">@{username}</p>
              </div>
            </div>
